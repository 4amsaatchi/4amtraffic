<?php   
  ...
  $query = "SELECT * FROM ordinthh ORDER BY fechahh desc LIMIT 150";  
  $resultone = mysqli_query($connecttwo, $query);
 function fill_brand($connect)  
 {  
      $output = '';  
      $sql = "SELECT * FROM ageperso LIMIT 2000";  
            ...
            <div id="order_table" class="panel panel-default row">  
               ...
               ?>  
                      <tr>  
                           ...
                           <td><?php echo $row["fechahh"]; ?></td>  
                           <td><?php echo $row["valhorah"]; ?></td>  
                           <td><?php echo $row["valminh"]; ?></td>  
                           <td><?php echo $row["tiptrahh"]; ?></td>  
                           <td><?php echo $row["responhh"]; ?></td> 
                      </tr>  
               <?php  
               }  
               ?>  
...
$("#order_table-col").tableExport({formats: ["xlsx"]});
 ...
 $(document).ready(function(){
      ...
      $('#filter').click(function(){  
        ...
        to_date = $('#to_date').val();
        codper = $('#customers').val();
        //console.log(codcli, from_date, to_date);
        if(codper != "" && from_date != '' && to_date != '')  {  
             $.ajax({  
                  url:"load_hh.php",  
                  method:"POST",  
                  data:{codper:codper, from_date:from_date, to_date:to_date},
                  beforeSend: function() {
