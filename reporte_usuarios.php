<?php
   ...
   function get_usuarios($connect) {
      $output = '';
      $sql = "SELECT DISTINCT agepersocli.*, climae.nomcli, ageperso.nomper, ageperso.activoper
      FROM agepersocli
         INNER JOIN ageperso ON agepersocli.codper = ageperso.codper
         INNER JOIN climae ON agepersocli.codcli = climae.codcli
      WHERE activoper = 'Activo'
      GROUP BY agepersocli.codper
      ORDER BY nomper ASC";

      $result = mysqli_query($connect, $sql);
      while($row = mysqli_fetch_array($result)) {  
         $output .= '<option value="'.$row["codper"].'">'.$row["nomper"].'</option>';  
      }  
      return $output;  
   }
...
<html lang="es">
  ...
  <script>
    $(document).ready(function() {
      $('#show_usuarios').select2({ placeholder: "Seleciona Usuario", allowClear: true});
      $('#show_agencias').select2({ placeholder: "Seleciona Agencia", allowClear: true});
      $('#show_customers').select2({ placeholder: "Seleciona cliente", allowClear: true});
      $('#order_table-col').tablesorter();
      $("#checkbox-user").click(function(){
          if($("#checkbox-user").is(':checked') ){
              $("#show_usuarios > option").prop("selected","selected");
              $("#show_usuarios").trigger("change");
          }else{
              $("#show_usuarios > option").removeAttr("selected");
               $("#show_usuarios").trigger("change");
           }
      });
      $("#checkbox-cliente").click(function(){
          if($("#checkbox-cliente").is(':checked') ){
              $("#show_customers > option").prop("selected","selected");
              $("#show_customers").trigger("change");
          }else{
              $("#show_customers > option").removeAttr("selected");
               $("#show_customers").trigger("change");
           }
      });
    });
...
<script>  
 $(document).ready(function(){  
      $('#show_usuarios').change(function(){  
           //var codagencia = $(this).val();
          usuarios='';

          valusuarios=document.getElementById('show_usuarios');
          for (i=0;i< valusuarios.length;i++) { 
            if(valusuarios[i].selected){
              usuarios +=  '"' + valusuarios[i].value + '",'; 
            }
          }         
          usuarios=usuarios.slice(0,usuarios.length -1);
          ...
          if (usuarios != "") {
            $.ajax({  
                ...
                success:function(data){  
                  $('#show_agencias').html(data);
                }  
            });
          } else {
            console.log("Selecione agencia");
          }
      });
      //Second Ajax
      $('#show_agencias').change(function(){  
           //var codagencia = $(this).val();
          usuarios='';
          ...
          valagencia=document.getElementById('show_agencias');
          for (i=0;i< valagencia.length;i++) { 
              if(valagencia[i].selected){
                  agencia +=  '"' + valagencia[i].value + '",'; 
              }
          }         
          agencia=agencia.slice(0,agencia.length -1);
          //console.log(agencia);
          //console.log(usuarios);
          
          if (usuarios != "" && agencia != "") {
            $.ajax({  
                url:"ajaxCustomers.php",  
                method:"POST",  
                data:{usuarios:usuarios, agencia:agencia},  
                success:function(data){  
                  $('#show_customers').html(data);  
                }  
            });
          } else {
            alert ("Selecione agencia");
          }
      });
 });
 </script>
 ...
 $(document).ready(function(){
      ...
      $('#filter').click(function(){
        ...
        val=document.getElementById('show_customers');
        ...
        for (i=0;i< val.length;i++) { 
            if(val[i].selected){
                usuaiodt +=  '"' + val[i].value + '",'; 
            }
        }         
        usuaiodt=usuaiodt.slice(0,usuaiodt.length -1);

        if(usuaiodt =! "" && agencia != "" && codcli != "" && from_date != '' && to_date != '')  {
          susuarios='';

          valsusuarios=document.getElementById('show_usuarios');
          for (i=0;i< valsusuarios.length;i++) { 
            if(valsusuarios[i].selected){
              susuarios +=  '"' + valsusuarios[i].value + '",'; 
            }
          }         
          susuarios=susuarios.slice(0,susuarios.length -1);
          console.log(susuarios);
             $.ajax({  
                  url:"user_data.php",  
                  method:"POST",  
                  data:{susuarios:susuarios, agencia:agencia, codcli:codcli, from_date:from_date, to_date:to_date},
                  beforeSend: function() {
